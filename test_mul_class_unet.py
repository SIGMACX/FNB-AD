...
def inference(num_classes, input_channels, snapshot_dir, save_path):
    test_dataset = FNDataset(
        ...
        label_path='./data/FN_local/testingset/test/masks/',
        split = 'test',
    ...
    fn_model = NestedUNet(input_channels, num_classes).to(device)
    ...
    for index, (img, img_mask, _, img_name, _) in enumerate(test_dataloader):
        img = img.to(device)
...
if __name__ == '__main__':
    ...
    inference(num_classes=args.num_classes,
              input_channels=3,
              snapshot_dir='./snapshots/NestedUNet_3/unet_model_2000.pkl',
              save_path=f'/data1/chenxi/code/medical_segmentation/Fetal_nasal_bone_detection/Result/mul_fn_seg/class_3/NestedUNet/mul_class/{current_time}')